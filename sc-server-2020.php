...
$y = (isset($_GET['year'])) ? (int) $_GET['year'] : 2020;
$y1 = $y - 1;
$y2 = $y + 1;
echo "
<div class='box3' style='left: 0px; top: 5px;border: 0;cursor:pointer' onclick=\"window.open('sc-server-2020.php?year=$y1', '_self')\">
<span class='textcentered'>&#10094;</span>
</div>
<div class='box5' style='left: 21px; top: 5px'>
<span id='donkey' class='textcentered year'>$y</span>
</div>
<div class='box3' style='left: 80px; top: 5px;border: 0;cursor:pointer' onclick=\"window.open('sc-server-2020.php?year=$y2', '_self')\">
<span class='textcentered'>&#10095;</span>
</div>";
...
for ($x = 0; $x < 40; $x++) {
    echo '<div class="box7" id="ass' . $x . '" style="left: ' . (100 + $x * 31) .
        'px; top: 5px; font-size: 8px; background: white;color: black"' .
        ' key="' . $x . '" onclick="select(this)" onmousedown="md()" onmouseup="mu()" onmouseout="mu()"><activity></activity></div>';
}
...
for ($m = 1; $m < 13; $m++) {
    ...
    for ($dd = 1; $dd < 32; $dd++) {
        if ($m == intval(date('n', mktime(0, 0, 0, $m, $dd, $y)))) {
            ...
            // render selectables
            for ($s = 0; $s < 4; $s++) {
                $key = "'" . sprintf("%4u.%02u.%02u.%04u", $y, $m, $dd, $s) . "'";
                $bor = ($s == 3) ? " border-right: solid black 2px;" : "";
                echo '<div class="box6" style="left: ' . ($xoff + 19 + $s * 31) . 'px; top: ' . $ypos . 'px' .
                ';' . $bor . $weekchange . $lastday .
                '" sel="' . $ii . '" onclick="change(this)' . '" id=' . $key .
                '><activity>' . $tta[0]->text . '</activity>';
                echo '</div>';
            }
        }
    }
...
?>
   ...
   function mads(ella) {
      if (cursel != 0) {
        ...
        xmlhttp.open("GET", "/controllers/update_scactivity.php?user=try&pos=" + n + "&fg=" + x.fg + "&bg=" + x.bg + "&fs=" + x.fs + "&text=" + x.text, true);
        xmlhttp.send();
...
function change(el) {
        ...
        var key = 'key=' + el.id;
        var sel = 'sel=' + cursel;
        xmlhttp.open("GET", "/controllers/change_activity.php?" + user + "&" + key + "&" + sel, true);
        xmlhttp.send();
        el.style.background = curselel.style.background;
        el.style.fontSize = curselel.style.fontSize;
        el.style.color = curselel.style.color;
        el.innerHTML = curselel.innerHTML;
        el.setAttribute('sel', cursel);
    }

    function refresh_from_scdata() {
        xmlhttp = new XMLHttpRequest();
...
//               console.log(this.responseText);
                ...
                for(var n=0;n<arr.length;n++){
                    var el = document.getElementById(arr[n].key);
                    var selel = document.getElementById('ass' + arr[n].activity);
                    el.style.background = selel.style.background;
                    el.style.fontSize = selel.style.fontSize;
                    el.style.color = selel.style.color;
                    el.innerHTML = selel.innerHTML;
                    el.setAttribute('sel', arr[n].activity);
                }
            }
        };
        var user = 'user=try';
        var key = 'year=' + document.getElementById("donkey").innerText;
        xmlhttp.open("GET", "/controllers/select_activities.php?" + user + "&" + key, true);
        xmlhttp.send();
    }

    function refresh_from_scactivities() {
        xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
//                console.log(this.responseText);
                var arr = JSON.parse(this.responseText);
                for(var n=0;n<arr.length;n++){
                    var el = document.getElementById('ass' + arr[n].x);
                    el.style.background = arr[n].bg;
                    el.style.fontSize = arr[n].fs;
                    el.style.color = arr[n].fg;
                    el.innerHTML = '<activity>' + arr[n].text + '</activity>';
                }
                refresh_from_scdata();
            }
        };
        var user = 'user=try';
        xmlhttp.open("GET", "/controllers/select_scactivities.php?" + user, true);
        xmlhttp.send();
    }
