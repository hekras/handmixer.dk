...
function animateloop() {
    ...
    if (grid.visible) {
        ...
        dc.beginPath();
        var xc = width / 2;
        var xnd = Math.floor(xc / grid.ddt / 5);
        var p = i0 + xnd * 5 * grid.ddt;
        dc.moveTo(p, 0);
        dc.lineTo(p, height);
        var yc = height / 2;
        var ynd = Math.floor(yc / grid.ddt / 5);
        var p = i0 + ynd * 5 * grid.ddt;
        dc.moveTo(0, p);
        dc.lineTo(width, p);
        dc.stroke();
    }
    ...
    for (var i = 0; i < sequence.length; i++) {
        var px = grid.ddt * (sequence[i].x + xnd * 5);
        var py = grid.ddt * (sequence[i].y + ynd * 5);
    ...
    mouse.dx = Math.floor((mouse.x + grid.ddt / 2) / grid.ddt) - xnd * 5;
    mouse.dy = Math.floor((mouse.y + grid.ddt / 2) / grid.ddt) - ynd * 5;
    var px = grid.ddt * (mouse.dx + xnd * 5);
    var py = grid.ddt * (mouse.dy + ynd * 5);
    dc.fillStyle = "yellow";
    dc.beginPath();
